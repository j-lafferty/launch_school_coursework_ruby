...
def display_result(longest)
  puts "The longest sentence word count: \n#{longest.split(' ').size}"
  puts "The longest sentence: \n'#{longest.strip}'"
end

def longest_sentince(text)
  sentences = text.gsub(/[.?!]/, '\0|')
                  .tr("\n", " ")
                  .split('|')

  longest = sentences.inject do |memo, sentence|
    memo.tr(' ','').length > sentence.tr(' ','').length ? memo : sentence
  end
...
longest_sentince('Four score and seven years ago our fathers brought forth
